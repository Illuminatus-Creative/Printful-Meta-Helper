...
final class PMH_Admin_Assets {
	...
	/**
	 * Data embedded in an inline <script>. HEX_TAG makes "</script>" in a
	 * blank name harmless regardless of slash escaping; HEX_AMP guards
	 * against entity tricks if the block is ever moved into an attribute.
	 */
	public const JSON_FLAGS = JSON_HEX_TAG | JSON_HEX_AMP;
	...
	public static function enqueue( $hook_suffix ): void {
		...
		if ( $on_blank_screen ) {
			...
			wp_add_inline_script( 'pmh-size-grid', 'window.pmhGridI18n = ' . wp_json_encode( self::grid_i18n(), self::JSON_FLAGS ) . ';', 'before' );
		}
		...
		if ( $on_product_screen ) {
			...
			wp_add_inline_script( 'pmh-blank-filter', 'window.pmhAssign = ' . wp_json_encode( PMH_Product_Meta::inline_data( $product_id ), self::JSON_FLAGS ) . ';', 'before' );
		}
	}
